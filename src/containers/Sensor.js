...
const PatientInfo = styled.div`width: 100%; padding: 0 3vw 0 3vw`
class Sensor extends Component {
    ...
    timer = (node) => {
        ...
        this.props.fetchSensorData(node)
            .then((err, callback) => {
    ...
    render() {
        ...
        let tableBody = (<tr><th scope="row">{this.props.patient.first_name}</th>{this.tableBody()}</tr>);
        let data = null;
        return (
            <PatientInfo>
                <Table tableHeadRow={tableHeadRow} tableBody={tableBody} />
                <SensorStream data={this.chartData()} />
                <select onChange={() => {this.onHandleChange()}} className="pull-right" id="timeInterval">
...
function mapStateToProps(state) {
    return { patient: state.patients.patient, sensor_data: state.hospitals.sensor_data };
}

export default connect(mapStateToProps, { fetchPatient, fetchSensorData })(Sensor);